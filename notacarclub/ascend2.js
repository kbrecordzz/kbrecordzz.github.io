...
else if(document.cookie.search("cookie_x2=")!==-1&&document.cookie.search("cookie_z2=").length!==-1)
{let kaka_x1=document.cookie.split("cookie_x2=");let kaka_x2=kaka_x1[1].split(";");start_chunk_x=parseInt(kaka_x2[0]);let kaka_z1=document.cookie.split("cookie_z2=");let kaka_z2=kaka_z1[1].split(";");start_chunk_z=parseInt(kaka_z2[0]);}
else
{start_chunk_x=37;start_chunk_z=40;}
...
{hm[fi1][fj1][fi][fj]-=100;terrain_make_plains(fi1,fj1,fi,fj,5);}
if((fi1===38&&fj1===27))
{hm[fi1][fj1][fi][fj]-=100;terrain_make_plains(fi1,fj1,fi,fj,5);if(distance_get_xz(fi,fj,41,9)<2)hm[fi1][fj1][fi][fj]*=1.3;else if(distance_get_xz(fi,fj,20,19)<2)hm[fi1][fj1][fi][fj]*=0.6;else if(distance_get_xz(fi,fj,24,40)<3)hm[fi1][fj1][fi][fj]*=1.3;else if(distance_get_xz(fi,fj,6,25)<2)hm[fi1][fj1][fi][fj]*=1.2;}
if(fi1===39&&fj1===27)
{hm[fi1][fj1][fi][fj]=5;}
if(fi1===38&&fj1===28)
{let mdist=distance_get_xz(fi,fj,24,22);if(mdist<11)hm[fi1][fj1][fi][fj]=vgrelf-0.5*mdist-467;}
...
{let rdist=distance_get_oval_xz(x_in_chunk_to_x(fi1,fi),x_in_chunk_to_x(fj1,fj),40*(chunkwidth-1)+25,29*(chunkwidth-1)+25,0.5,2);if(rdist>=41&&rdist<=50)hm[fi1][fj1][fi][fj]=sealevel+0.3;else hm[fi1][fj1][fi][fj]=sealevel-12;}}}
...
{om[fi1][fj1][fi][fj]=0;if(fi===45||fi===4||fj===45||fj===4)om[fi1][fj1][fi][fj]=ASCEND_SKYSCRAPER;else if(fi%3<(2*pseudorandom(fi*fj))&&fj%4<(2*pseudorandom(fi+fj))&&(grelf()>315||grelf_detail()>487||grelf3()>47))om[fi1][fj1][fi][fj]=ASCEND_BARN;if(fi%23===0&&fj%17===0)om[fi1][fj1][fi][fj]=ASCEND_SKYSCRAPER;}
else if((fi1===39&&fj1===27)&&fj<46&&fj>3&&fi>3&&fi<46)
{if(fi===45||fi===4||fj===45||fj===4)om[fi1][fj1][fi][fj]=ASCEND_SKYSCRAPER;else if(fi%7===2)om[fi1][fj1][fi][fj]=ASCEND_SKYSCRAPER;}
else if((fi1>=39&&fi1<=41)&&(fi1>=28&&fj1<=30))
{let xx=x_in_chunk_to_x(fi1,fi);let zz=x_in_chunk_to_x(fj1,fj);let centerx=40*(chunkwidth-1)+25;let centerz=29*(chunkwidth-1)+25;let rdist=distance_get_oval_xz(xx,zz,centerx,centerz,0.5,2);if(rdist>=41&&rdist<=50)om[fi1][fj1][fi][fj]=ASCEND_ROAD;else om[fi1][fj1][fi][fj]=0;}}
else if((fi1>=32&&fi1<=35)&&(fj1>=36&&fj1<=38))
...
{mat_houses=tex("lyxs.jpg");mat_houses.color=new THREE.Color(0x88E3F9);mat_skyscrapers=tex("house_glass.jpg");mat_barns=tex("house_glass.jpg");mat_barns.transparent=true;mat_barns.opacity=0.6;mat_trees=psp("tree_sea.png");mat_bushes=psp("bluepine.png");mat_grass=psp("bluetree.png");mat_terrain=tex("phaser_volcano.jpg");if((fi1>=38&&fi1<=39)&&fj1===33)
{}
...
{mat_houses=tex("prisonwall.jpg");mat_skyscrapers=tex("skyscraper1.jpg");mat_barns=tex("prisonwall.jpg");mat_trees=psp("krets_tree.png");mat_bushes=psp("tree_desert.png");mat_grass=psp("bush_magnet.png");mat_terrain=tex("phaser_volcano.jpg");if(fi1===38&&fj1===28)
{mat_skyscrapers=tex("trainwall.jpg");mat_barns=tex("roomwall.png");}
else if(fi1>=39&&fi1<=41&&fj1>=28&&fj1<=30)
{mat_trees=psp("hongtree.png");mat_bushes=psp("greentree.png");mat_grass=psp("handtree9.png");}
...
{}
if((fi1>=39&&fi1<=41)&&(fi1>=28&&fj1<=30))mat_terrain=tex("");else if(fi1===38&&fj1===28){mat_terrain=tex("phaser_bluegreen.jpg");mat_terrain.color=new THREE.Color(0xFF2263);}
...
create_3d_meshes(i1,j1);objects_is_loaded[i1][j1]=1;if(i1===27&&j1===36)
{for(let t=0;t<45;t++)
{for(let u=0;u<45;u++)
{ver_aker.push(27*49+t+3+0.4,height_get_xz_exact(27*49+t+3+0.4,36*49+u+3+0.4)+0.2,36*49+u+3+0.4);}}
geometry_aker=new THREE.BufferGeometry();geometry_aker.setAttribute('position',new THREE.Float32BufferAttribute(ver_aker,3));mat_aker=psp("grass_aker.png",2);pointsprites_aker=new THREE.Points(geometry_aker,mat_aker);scene.add(pointsprites_aker);ver_aker.length=0;}}}
...
{create_3d_meshes(to_load_chunk_x[dynload_objects_to_load_i],to_load_chunk_z[dynload_objects_to_load_i]);objects_is_loaded[to_load_chunk_x[dynload_objects_to_load_i]][to_load_chunk_z[dynload_objects_to_load_i]]=1;if(to_load_chunk_x[dynload_objects_to_load_i]===27&&to_load_chunk_z[dynload_objects_to_load_i]===36)
{for(let t=0;t<45;t++)
{for(let u=0;u<45;u++)
{ver_aker.push(27*49+t+3+0.4,height_get_xz_exact(27*49+t+3+0.4,36*49+u+3+0.4)+0.2,36*49+u+3+0.4);}}
geometry_aker=new THREE.BufferGeometry();geometry_aker.setAttribute('position',new THREE.Float32BufferAttribute(ver_aker,3));mat_aker=psp("grass_aker.png",2);pointsprites_aker=new THREE.Points(geometry_aker,mat_aker);scene.add(pointsprites_aker);ver_aker.length=0;}}
