...
for (let uvi = 0; uvi < 28812; uvi += 12)
{
	...
	uv_terrain[uvi+6]	= (_1D49+(_1D49)*(uviD12Mcurrent_lineX49));	uv_terrain[uvi+7]	= (current_line*(_1D49));		// 4 (√§r samma som 1)		// gˆr denna ˆverflˆdig?
	uv_terrain[uvi+8]	= (_1D49+(_1D49)*(uviD12Mcurrent_lineX49));	uv_terrain[uvi+9]	= (_1D49+current_line*(_1D49));		// 5
	uv_terrain[uvi+10]	= (_1D49*(uviD12Mcurrent_lineX49));		uv_terrain[uvi+11]	= (_1D49+current_line*(_1D49));		// 6 (√§r samma som 2)		// gˆr denna ˆverflˆdig?
...
for (let uvi = 0; uvi < 30000; uvi += 84)
{
	...
	uv_houses[uvi+74] = 0.25; uv_houses[uvi+75] = 0.5;	// l√§ngst upp p√• tak"nocken"
	uv_houses[uvi+76] = 0.5; uv_houses[uvi+77] = 0;

	uv_houses[uvi+78] = 0.5; uv_houses[uvi+79] = 0;
	uv_houses[uvi+80] = 0.25; uv_houses[uvi+81] = 0.5;	// l√§ngst upp p√• tak"nocken"
...
//	if (isNaN(p_value)) p_value = hm[ixf][izf][xf][zf];			// tar bort denna sÂ det blir error ist‰llet! 231013
...
function set_wideness_highness(fi1, fj1)
{
	...
	else if ((fi1 >= 32 && fi1 <= 35) && (fj1 >= 36 && fj1 <= 38)) {	highness = 15;		wideness = 4;	seed = 0.6602099002976599;	weights_seed = 2; }		// tj√∂rn
	else if ((fi1 >= 29 && fi1 <= 30) && (fj1 >= 35 && fj1 <= 40)) {	highness = 25;		wideness = 1.5;	seed = 0.6511030308479817; 	weights_seed = 2; }		// norway
	else if ((fi1 >= 31 && fi1 <= 35) && (fj1 >= 39 && fj1 <= 41)) {	highness = 20;		wideness = 3;	seed = 0.6511030308479817; 	weights_seed = 2; }		// h√•rass land
	...
	// denna kommer nu s√§ttas varje g√•ng, men det √§r luuugnt f√∂r den s√§tts bara en g√•ng per chunk och inte en g√•ng per block i chunken
...
{
	// h√§r kan man r√§kna ut n√•gra fi1 och fj1 i b√∂rjan! men se till att det g√att!
...
else
{
	...
	let vgrelf = grelf();
	let vgrelf3 = grelf3();
	hm[fi1][fj1][fi][fj] = vgrelf3*highness*0.005;		// testar att kˆra ALLT pÂ master > 0.5! se om nÂgot blir fel! 231002
	...
	if ((fi1 >= 36 && fi1 <= 39) && (fj1 >= 33 && fj1 <= 35))
	{
		...
		if ((fi1 === 36 && fj1 === 34) && (fj > 22 && fj < 28))		hm[fi1][fj1][fi][fj] -= 2;	// "ing√•ng"
	...
	else if ((fi1 >= 36 && fi1 <= 41) && (fj1 >= 27 && fj1 <= 32))
	{
		...
		if (fj1 === 32)							hm[fi1][fj1][fi][fj] -= 1.5 * fj;
										...
										hm[fi1][fj1][fi][fj] = 2*Math.floor(0.5*hm[fi1][fj1][fi][fj]);	// bara j√§mna steg
		...
		if (fi1 >= 39 && fi1 <= 41 && fj1 >= 28 && fj1 <= 30)
		{
			...
			if (fi1 === 40 && fj1 === 29)
			{
				let planety = Math.sqrt(23*23 - (fi-25)*(fi-25) - (fj-25)*(fj-25)) - 5;		// 23 = radie, 25 = center. 5 = neds√§nkning
	...
	else if ((fi1 >= 33 && fi1 <= 35) && (fj1 >= 28 && fj1 <= 32))
	{
		...
		if (fj1 === 28 && fj < 20)					hm[fi1][fj1][fi][fj] -= 0.05 * (20-fj)*(20-fj);	// slut p√• hela skiten
	...
	else if ((fi1 >= 32 && fi1 <= 35) && (fj1 >= 36 && fj1 <= 38))
	{
		...
		if ((fi1 >= 32 && fi1 <= 35) && (fj1 >= 36 && fj1 <= 38))	hm[fi1][fj1][fi][fj] *= 0.12;				// tj√∂rn
		if ((fi1 >= 32 && fi1 <= 35) && (fj1 >= 36 && fj1 <= 38))	hm[fi1][fj1][fi][fj] += 10-9.248347-1;	// tj√∂rn
		...
		if ((fi1 === 32 && fj1 === 38) && distance_get_xz(fi,fj, 42,32) < 2) hm[fi1][fj1][fi][fj] = sealevel-0.5;		// gym sj√∂
	...
	else if ((fi1 >= 29 && fi1 <= 30) && (fj1 >= 35 && fj1 <= 40))
	{
		...
		// Denna g√∂r att det blir h√∂gre berg ju mer √•t z-riktninig man √•ker: 0.003*((fj1*49+fj)-35*49)
		...
		if (fj1 === 40)							terrain_amplify(fi1,fj1,fi,fj, -0.5);		// lite coola berg & sj√∂, av n√•n anledning
	...
	else if ((fi1 >= 31 && fi1 <= 35) && (fj1 >= 39 && fj1 <= 41))
	{
		...
		{
			if ((fi+fj) % 4 === 0) hm[fi1][fj1][fi][fj] -= vgrelf_detail*0.001;	// sj√∂
			terrain_amplify_4chunks(33,39,fi1,fj1,fi,fj, -0.7, 1.3);			// sj√∂ forts.
		}

		terrain_amplify(fi1,fj1,fi,fj, (pseudorandom(fi1*fj1)*2)-1);	// small random differences

		if ((fi1 >= 31 && fi1 <= 35) && (fj1 >= 39 && fj1 <= 41))	hm[fi1][fj1][fi][fj] *= (0.5*factor_chunkmap);		// h√•rass land
		if ((fi1 >= 31 && fi1 <= 35) && (fj1 >= 39 && fj1 <= 41))	hm[fi1][fj1][fi][fj] += 10-9.248347;			// h√•rass land

		// flod-delta
		hm[fi1][fj1][fi][fj] *= 0.01*((fi1*49+fi)-31*49);		// mer land ju mer √•t z-h√•llet
		hm[fi1][fj1][fi][fj] -= 2;
		let tmp = hm[fi1][fj1][fi][fj] - sealevel;			// utj√§mna runt sealevel s√• det blir platt men land/vatten-f√∂rh√•llandet blir samma
		tmp *= 0.03;
		hm[fi1][fj1][fi][fj] = sealevel + tmp;

		if (fi1 >= 34)
		{
										hm[fi1][fj1][fi][fj] += 0.02*((fi1*49+fi)-34*49);	// h√∂gre ju mer √•t z-h√•llet

			// kinesiska risf√§lt-"backar"
										hm[fi1][fj1][fi][fj] += 5;
										hm[fi1][fj1][fi][fj] *= 20;
										hm[fi1][fj1][fi][fj] -= 85;
		...
		if (fi1 === 33 && fi >= 40)					hm[fi1][fj1][fi][fj] -= 0.1 * (fi-40)*(fi-40);	// slut p√• sanddyner
		if (fi1 === 35)							hm[fi1][fj1][fi][fj] -= 0.015 * fi*fi;	// slut p√• hela skiten
	...
	else if ((fi1 >= 36 && fi1 <= 39) && (fj1 >= 36 && fj1 <= 41))
	{
		...
		if ((fi1 >= 36 && fi1 <= 39) && (fj1 >= 36 && fj1 <= 41))	hm[fi1][fj1][fi][fj] += 5-9.248347;	// haftlan-drakh. konstigt tal √§r f√∂r att fixa efter sealevel-fel

		if ((fi1 === 39 && fj1 === 38) && (fi > 1 && fi < 16 && fj > 20 && fj < 36) && (hm[fi1][fj1][fi][fj] > 10 && hm[fi1][fj1][fi][fj] < 20)) hm[fi1][fj1][fi][fj] = 14;	// plat√• (dark gandalf)
		if ((fi1 === 37 && fj1 === 37) && distance_get_xz(fi,fj, 25,30) < 8) hm[fi1][fj1][fi][fj] = sealevel-10+0.03*vgrelf_detail;	// √∂ i sj√∂?

		if (fi1 === 37 && fj1 === 41) terrain_amplify(fi1,fj1,fi,fj, -1);	// sjˆ
	...
	else if ((fi1 >= 29 && fi1 <= 32) && (fj1 >= 28 && fj1 <= 34))
	{
		...
		if ((fi1 === 30 && fj1 === 29) && distance_get_xz(fi,fj, 18,13) < 9)	hm[fi1][fj1][fi][fj] = 9.3;					// upph√∂jt/neds√§nkt (magnet festival)
...
// tjørn klubbhus-hemlig gøng in i berget
if (fi1 === 34 && fj1 === 37)
{
hm[34][37][28][20] = 0.6;
hm[34][37][27][11] = 0.6;
...
// haftlan-drakh bro øver lila sjøn
if (fi1 === 37 && fj1 === 37)
{
...
// hørassland stavanger liten damm
...
// epper bridge lite smø piggar ute i havet
if (fi1 === 33 && fj1 === 34)
{
hm[33][34][0][12] = 0.0;
hm[33][34][6][6] = 0.0;
...
// hongkong/japan liten hemlig gøng
...
// hongkong/japan nationalpark park sjø
if (fi1 === 29 && fj1 === 30)
{
...
// leaves parkering før amerikanare
if (fi1 === 31 && fj1 === 38)
{
...
// hongkong/japan golf hemlig gøng
...
//	else if ((fi1 >= 31 && fi1 <= 35) && (fj1 >= 39 && fj1 <= 41))
	...
	{
		if (fi1 === 31 && fj1 === 33 && fi % 2 === 0 && fj % 2 === 0 && distance_get_xz(fi,fj, 25,25) < 4) om[fi1][fj1][fi][fj] = ASCEND_TREE;	// √∂dsk√∂lt √∂
		else if (fi1 === 30 && fj1 === 29)
		{
			let fdist = distance_get_xz(fi,fj, 18,13);
			if (fdist < 9)	om[fi1][fj1][fi][fj] = 0;			// magnet festival
			else if (fdist >= 9 && fdist <= 11)	om[fi1][fj1][fi][fj] = ASCEND_TREE;					// upph√∂jt/neds√§nkt (magnet festival)
		}
...
//	else if ((fi1 >= 26 && fi1 <= 27) && (fj1 >= 26 && fj1 <= 31))
	...
	if (curve_objects < (-0.1/master2)+2 && hm[fi1][fj1][fi][fj] > sealevel+1
	    && (((fi1*3/fj1/fi*2/fj)*100)-(Math.floor((fi1*3/fj1/fi*2/fj)*100)) <= 0.6)	// n√•got fr√•n itch release som tar bort lite hus
	...
	//	else if (fi1 === 32 && fj1 === 35)
		...
		if (curve_houses <= 0.5)
		{
			...
			if (c+l+r+m <= 5)
			{
				...
				{
					if ((fi >= 4 && fi <= 45) && (fj >= 4 && fj <= 45))	// 45 = chunkwdith-5. detta f√∂r att det ska se bra ut n√§r hus t√§nds och sl√§cks n√§r man √•ker in i nya chunks
					{
						// variation
						if (fi1 % 9 < 3 && fj1 % 3 === 0) {}
						else if ((fi1 >= 31 && fi1 <= 35) && (fj1 >= 39 && fj1 <= 41)) {} // h√•rass land
...
//	else if ((fi1 >= 33 && fi1 <= 35) && (fj1 >= 33 && fj1 <= 35))
	...
	if ((fi1 >= 36 && fi1 <= 41) && (fj1 >= 27 && fj1 <= 32))
	{
		...
		{
			om[fi1][fj1][fi][fj] = 0;
			if (fi === 45 || fi === 4 || fj === 45 || fj === 4 || fi === 22 || fj === 22) om[fi1][fj1][fi][fj] = ASCEND_SKYSCRAPER;	// v√§ggar
			if (fj === 45 && fi === 3)				om[fi1][fj1][fi][fj] = 0;			// d√∂rr
			// d√∂rr ut
		}
		...
		{
			om[fi1][fj1][fi][fj] = 0;
			if (fi === 45 || fi === 4 || fj === 45 || fj === 4)	om[fi1][fj1][fi][fj] = ASCEND_SKYSCRAPER;	// v√§ggar
			// pelare i mitten
		...
		else if ((fi1 === 39 && fj1 === 27) && fj < 46 && fj > 3 && fi > 3 && fi < 46)
		{
			...
			else if (fi % 7 === 2)					om[fi1][fj1][fi][fj] = ASCEND_SKYSCRAPER;  // v√§ggar
			// d√∂rr in
			// d√∂rrar ut
			// d√∂rrar mellan korridorer
		}
...
//	else if ((fi1 >= 33 && fi1 <= 35) && (fj1 >= 28 && fj1 <= 32))
	...
	else if ((fi1 >= 32 && fi1 <= 35) && (fj1 >= 36 && fj1 <= 38))
	{
		...
		else if (fi1 === 33 && fj1 === 38)	// gym b√∂rjan
		{
...
// hørassland stavanger lite trød
...
// hongkong/japan nationalpark nøgra hus
if (fi1 === 30 && fj1 === 30)
{
om[30][30][0][21] = 70;
om[30][30][0][19] = 70;
...
// tjørn vøgen in till parkeringen
if (fi1 === 34 && fj1 === 37)
{
...
function create_3d_meshes(fi1, fj1)
{
	...
	mat_terrain = tex("phaser_volcano.jpg");		// s√§tter denna f√∂r att kunna l√§gga p√• en grund-f√§rg eller n√•t
...
function create_one(fi1, fj1)
{
	...
	for (let i = 0; i < 49; i++)
	{
		for (let j = 0; j < 49; j++)
		{
			...
			// det h‰r ‰r enda s‰ttet hittills som jag lyckats fÂ v‰gen att k‰nna av sin egen riktning - OMGJORD
...
// roads can't be confusing!!
	...
	// epper land race
	// gˆr v‰garna i epper land ej fˆrvirrande!! man mÂste fˆrstÂ vilket hÂll man ska starta Ât!!
	// inga branta backar, inte mÂl-v‰g sÂ n‰ra annan v‰g, platta till marken vid v‰g sÂ att man fattar att det ‰r en v‰g. fler hus som markerar. helt nytt race
	// inte fˆr lÂngt mellan checkpoints, fˆr dÂ blir det wrongway!!
...
//	race_create(x_array, z_array, 3);
	...
	// hongkong/japan dogert √∂vningsplats1
		...
		if (dynload_hm_to_load_i < 25)
		{
			...
			else
			{
				...
				if (i_continue < chunkwidth)
				{
					...
					// ----
					// jag tror det ibland blir fel (spelet l√•ser sig) n√§r man √•kt en bit
					// f√∂r att hm- och objects-array:erna skapas dynamiskt.
					// om de inte hunnit skapas n√§r man ska h√§mta data fr√•n dem s√• l√•ses spelet.
					// d√§rf√∂r kollar jag om array:en har skapats f√∂rst h√§r
...
function create_terrain_chunks_before_showing_them()
{
	...
	if (ci1 !== last_chunk_x || cj1 !== last_chunk_z)
	{
		...
		let n = 1;		// anv‰nds inte till nÂgot?
		for (let t = ci1-2; t <= ci1+2; t++)
		{
			for (let u = cj1-2; u <= cj1+2; u++)
			{
